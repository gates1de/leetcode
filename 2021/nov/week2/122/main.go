...
func ngSolution(prices []int) int {
	...
	for i, _ := range prices {
		m[len(prices) - i] = []int{-1, -1}
	}
	helper(-1, 0, prices, m, &result)
	return result
}

func helper(hold int, current int, prices []int, m map[int][]int, result *int) {
	// fmt.Printf("hold = %v, current = %v, m = %v, result = %v\n", hold, current, m, *result)
	if len(prices) == 0 {
		if *result < current {
			*result = current
		}
	...
	if hold == -1 {
		if m[len(prices)][0] >= 0 && current <= m[len(prices)][0] {
			return
		}
		m[len(prices)][0] = current
	} else if hold >= 0 {
		if m[len(prices)][1] >= 0 && current <= m[len(prices)][1] {
			return
		}
		m[len(prices)][1] = current
	}

	price := prices[0]
	if hold == -1 {
		// buy
		helper(price, current - price, prices[1:], m, result)
	} else if hold < price {
		// sell and buy
		helper(price, current, prices[1:], m, result)
		// sell
		helper(-1, current + price, prices[1:], m, result)
	}
	// ignore
	helper(hold, current, prices[1:], m, result)
}
