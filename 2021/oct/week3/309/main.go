...
func helper(profit int, numOfStock int, dp map[[2]int]int, prices []int, result *int) {
	// fmt.Printf("profit = %v, numOfStock = %v, prices = %v\n", profit, numOfStock, prices)
	if len(prices) == 0 {
		if *result < profit {
			*result = profit
		}
	...
	if dp[[2]int{len(prices), numOfStock}] > 0 && profit <= dp[[2]int{len(prices), numOfStock}] {
		return
	}

	dp[[2]int{len(prices), numOfStock}] = profit

	// nothing
	helper(profit, numOfStock, dp, prices[1:], result)
	// buy
	if numOfStock == 0 {
		helper(profit - prices[0], 1, dp, prices[1:], result)
	}
	...
	if len(prices) == 1 {
		helper(profit + prices[0], 0, dp, prices[1:], result)
	} else {
		helper(profit + prices[0], 0, dp, prices[2:], result)
	}
}
